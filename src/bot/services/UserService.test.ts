...
describe("UserService", () => {
  ...
  it("should retrieve a submission by user ID", async () => {
    ...
    mockUserRepository.findSubmissionByUserId = vi
      .fn()
      .mockResolvedValue(submission);

    const result = await userService.getSubmissionByUserId(userId);

    expect(mockUserRepository.findSubmissionByUserId).toHaveBeenCalledWith(
      userId,
    );
  ...
  it("should return null if no submission is found", async () => {
    const userId = 1;
    mockUserRepository.findSubmissionByUserId = vi.fn().mockResolvedValue(null);

...
export class MusicGameService {
   ...
   */
  async showPlayerHelp(ctx: CommandContext): Promise<void> {
...
• Играйте честно и получайте удовольствие! 🎶`;
   ...
   */
  async showOrganizerHelp(ctx: CommandContext): Promise<void> {
...
• Следите за прогрессом через /music_stats`;
   ...
   */
  async showDeveloperHelp(ctx: CommandContext): Promise<void> {
    const helpText = `🔧 <b>Музыкальная игра - Техническая документация</b>

<b>Архитектура:</b>
• Модульная структура с разделением на features (lobby, gameplay, info)
...
• <code>SchedulerService</code> - планирование задач (подсказки, переходы)
...
• <code>REVEALED</code> - раунд завершён, ответ раскрыт

<b>База данных:</b>
• <code>Game</code> - игра (статус, настройки, чат)
• <code>Round</code> - раунд (трек, игрок, фаза)
• <code>Guess</code> - догадка (игрок, угаданный игрок, очки)
• <code>Hint</code> - сохранённая подсказка
...
• Примеры: <code>guess:roundId:userId</code>, <code>toggle:autoAdvance</code>

<b>Тестирование:</b>
• <code>game-flow.test.ts</code> - интеграционные тесты игрового флоу
• Используется Vitest для тестирования
• InMemoryGameRepository для мокирования БД

<b>Логирование:</b>
• Используется <code>console.log</code> для отладки
• Логируются ключевые события (создание игры, обработка догадок)
...
<b>Файлы:</b>
• <code>src/modules/musicGame/</code> - основной модуль
• <code>prisma/schema.prisma</code> - схема БД
• <code>locales/ru.json</code> - локализация`;

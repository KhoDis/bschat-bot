...
export class TriggerModule extends Composer<IBotContext> {
  ...
  private loadResponses() {
    ...
    this.triggers = [
      ...
      make('goodNight', /(спокойной\s+ночи|споки\s*ноки|(иду|я) спать|пош[ёе]л спать)/i),
      make('praise', /(молодец|горжусь|ты\s+сможешь|умничк[ауи]|верю в тебя|гордимся)/i),
      make('sad', /(мне\s+)?грустн[оы]|печаль|я\s+плачу|тоск[ауе]|хочется реветь|хочется плакать/i),
      make('hungry', /(хочу (есть|жрать)|проголодал[а-я]{0,2}|поесть бы|голодн[ыаяое])/i),
      make('weekend', /(пятниц[аы]|выходн(ой|ые)|отдыхаю|тус[аоу]ю|чиллю|релакс)/i),
  ...
  private generateResponse(topic: string): string {
    ...
    return [getRandom(parts.hook), getRandom(parts.punch), getRandom(parts.final)].join('\n');
  }

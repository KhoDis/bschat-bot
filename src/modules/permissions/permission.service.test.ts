...
describe('PermissionService', () => {
  ...
  it('should not grant a permission if role does not exist in a chat', async () => {
    ...
    await expect(permissionService.grantPermission(roleId, 'MANAGE_ROLES')).rejects.toThrow(
      'Role not found',
    );
  });
  ...
  it('should not revoke a permission if role does not exist in a chat', async () => {
    ...
    await expect(permissionService.revokePermission(roleId, 'MANAGE_ROLES')).rejects.toThrow(
      'Role not found',
    );
  });
  ...
  it('should handle permissions for roles in different chats independently', async () => {
    ...
    const result2 = await permissionService.hasPermission(roleId2, 'MANAGE_ROLES');

...
export class RoleModule extends Composer<IBotContext> {
  constructor(
    ...
    this.command('list_role_permissions', this.handleListRolePermissions.bind(this));
    ...
    this.command('list_available_permissions', this.handleListAvailablePermissions.bind(this));
  }
  ...
  private async handleListAllRoles(ctx: CommandContext) {
    ...
    try {
      ...
      if (error instanceof Error) {
        await ctx.reply(this.text.get('roles.list.error', { error: error.message }));
        return;
      }
    }
  }
  ...
  private async handleGrantPermission(ctx: CommandContext) {
    ...
    try {
      ...
      if (!Object.keys(PERMISSIONS).includes(permissionName)) {
        await ctx.reply(this.text.get('roles.grant.invalidPermission', { permissionName }));
        return;
      }
      ...
      await ctx.reply(this.text.get('roles.grant.success', { roleName, permissionName }));
    } catch (error) {
      if (error instanceof Error) {
        await ctx.reply(this.text.get('roles.grant.error', { error: error.message }));
        return;
      }
    }
  }
  ...
  private async handleRevokePermission(ctx: CommandContext) {
    ...
    try {
      // Validate permission name
      if (!Object.keys(PERMISSIONS).includes(permissionName)) {
        await ctx.reply(this.text.get('roles.revoke.invalidPermission', { permissionName }));
        return;
      }
      ...
      await ctx.reply(this.text.get('roles.revoke.success', { roleName, permissionName }));
    } catch (error) {
      if (error instanceof Error) {
        await ctx.reply(this.text.get('roles.revoke.error', { error: error.message }));
        return;
      }
    }
  }
  ...
  async handlePingRole(ctx: CommandContext): Promise<void> {
    ...
    try {
      ...
      if (error instanceof Error) {
        await ctx.reply(this.text.get('roles.ping.error', { error: error.message }));
        return;
      }
    }
  }
  ...
  private async handleAddRole(ctx: CommandContext) {
    ...
    try {
      ...
      if (error instanceof Error) {
        await ctx.reply(this.text.get('roles.add.error', { error: error.message }));
        return;
      }
    }
  }
  ...
  private async handleRemoveRole(ctx: CommandContext) {
    ...
    try {
      ...
      if (error instanceof Error) {
        await ctx.reply(this.text.get('roles.remove.error', { error: error.message }));
        return;
      }
    }
  }
  ...
  private async handleAssignRole(ctx: CommandContext) {
    ...
    try {
      ...
      if (error instanceof Error) {
        await ctx.reply(this.text.get('roles.assign.error', { error: error.message }));
        return;
      }
    }
  }
  ...
  private async handleListRoles(ctx: CommandContext) {
    ...
    try {
      ...
      const roles = await this.roleService.getUserRoles(BigInt(targetUser.id), BigInt(chatId));
      ...
      if (error instanceof Error) {
        await ctx.reply(this.text.get('roles.userRoles.error', { error: error.message }));
        return;
      }
    }
  }
